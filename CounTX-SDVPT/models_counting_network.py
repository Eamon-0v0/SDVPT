...
class CountingNetwork(nn.Module):
    def __init__(
        ...
        self.clip_model = open_clip.create_model(
            "ViT-B-16", pretrained="./pretrained/open_clip_pytorch_model.bin"
        )
    ...
    def forward_img_encoder(self, imgs,mode,class_ids,sim_list,class_index_list):
        return self.clip_model.encode_image(imgs,mode,class_ids,sim_list,class_index_list)
    ...
    def forward(self, imgs, counting_queries,mode,class_ids,train_txt_tokens):
        # with torch.no_grad():
        ...
        if mode=='train':
            ...
            sim = F.cosine_similarity(txt_tokens, train_txt_tokens , dim=-1)  # [32, 89]
            ...
            sim_normalized, class_index_list = torch.topk(sim_normalized, 80)
            sim_sum = sim_normalized.sum(dim=1, keepdim=True)  # [32, 1]
            sim_list = sim_normalized / sim_sum  # [32, 89]
            
        img_tokens,outs_vpt_class,outs_vpt_weighted  = self.forward_img_encoder(imgs,mode,class_ids,sim_list,class_index_list)
        ...
        return pred,outs_vpt_class,outs_vpt_weighted
