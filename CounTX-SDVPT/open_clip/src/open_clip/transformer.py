...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.vpt_depth = 10
        self.vpt_width = 20
        self.class_num=150 # Not all were used, only visual prompts equal to the number of training categories in FSC-147-D were used (i.e. 123)
        self.vpt_dropout = nn.Dropout(0.1)
        self.vpt_norm = nn.LayerNorm(width, eps=1e-6)
        self.vpt_proj = nn.Linear(width, width)
        nn.init.kaiming_normal_(self.vpt_proj.weight, a=0, mode='fan_out') 

        val = math.sqrt(6. / float(3 * functools.reduce(operator.mul, to_2tuple(patch_size), 1) + width))  
        vpt = torch.empty((self.class_num,self.vpt_depth, self.vpt_width, width))
    ...
    def forward(self, x: torch.Tensor,mode,class_ids,sim_list,class_index_list):
        ...
        x = self.deep_vpt_forward(x,mode,class_ids,sim_list,class_index_list,outs_vpt_class,outs_vpt_weighted)
        ...
        return x[:, 1:],outs_vpt_class,outs_vpt_weighted
