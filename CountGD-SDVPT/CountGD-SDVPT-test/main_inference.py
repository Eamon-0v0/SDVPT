...
from engine_inference import evaluate
...
def get_args_parser():
    ...
    parser.add_argument(
        "--num_exemplars", default=0, type=int, help="Set to 0 to use only text~~~"
    )
...
def main(args):
    ...
    if args.freeze_keywords is not None:
        for name, parameter in model.named_parameters():
            for keyword in args.freeze_keywords:
                if keyword in name and 'prompt' not in name:
    ...
    if args.eval:
        ...
        print("This code can only be used to inference, use --eval in command")
        return
