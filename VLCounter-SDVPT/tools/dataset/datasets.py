...
class listDataset(Dataset):
    def __init__(self, data_root, prompt, singular, mode, main_transform):
        ...
        self.other_class_index_dic={'ants': 0, 'apples': 1, 'birds': 2, 'books': 3, 'bottle caps': 4, 'bullets': 5, 'camels': 6, 'candy pieces': 7, 'carrom board pieces': 8, 'cashew nuts': 9, 'chairs': 10, 'chicken wings': 11, 'comic books': 12, 'crab cakes': 13, 'deers': 14, 'donuts tray': 15, 'eggs': 16, 'elephants': 17, 'finger foods': 18, 'flamingos': 19, 'flower pots': 20, 'flowers': 21, 'fresh cut': 22, 'grapes': 23, 'green peas': 24, 'horses': 25, 'hot air balloons': 26, 'keyboard keys': 27, 'kiwis': 28, 'legos': 29, 'marbles': 30, 'markers': 31, 'milk cartons': 32, 'nail polish': 33, 'oyster shells': 34, 'oysters': 35, 'peaches': 36, 'pills': 37, 'polka dots': 38, 'potato chips': 39, 'prawn crackers': 40, 'red beans': 41, 'sauce bottles': 42, 'sausages': 43, 'sea shells': 44, 'seagulls': 45, 'shallots': 46, 'sheep': 47, 'shirts': 48, 'skateboard': 49, 'skis': 50, 'stamps': 51, 'sticky notes': 52, 'strawberries': 53, 'sunglasses': 54, 'toilet paper rolls': 55, 'tree logs': 56, 'watches': 57}
    ...
    def __getitem__(self, index):
        ...
        if self.mode=='train':
            class_ids=self.train_class_index_dic[class_chosen]
        else:
            class_ids=self.other_class_index_dic[class_chosen]
        ...
        return query_img, query_density, tokenized_text, class_chosen, class_ids



def get_train_token(prompt):
    train_name_list=['alcohol bottles', 'baguette rolls', 'balls', 'bananas', 'beads', 'bees', 'birthday candles', 'biscuits', 'boats', 'bottles', 'bowls', 'boxes', 'bread rolls', 'bricks', 'buffaloes', 'buns', 'calamari rings', 'candles', 'cans', 'caps', 'cars', 'cartridges', 'cassettes', 'cement bags', 'cereals', 'chewing gum pieces', 'chopstick', 'clams', 'coffee beans', 'coins', 'cotton balls', 'cows', 'cranes', 'crayons', 'croissants', 'crows', 'cupcake tray', 'cupcakes', 'cups', 'fishes', 'geese', 'gemstones', 'go game', 'goats', 'goldfish snack', 'ice cream', 'instant noodles', 'jade stones', 'jeans', 'kidney beans', 'kitchen towels', 'lighters', 'lipstick', 'm&m pieces', 'macarons', 'matches', 'meat skewers', 'mini blinds', 'mosaic tiles', 'naan bread', 'nails', 'nuts', 'onion rings', 'oranges', 'pearls', 'pencils', 'penguins', 'pens', 'people', 'peppers', 'pigeons', 'plates', 'polka dot tiles', 'potatoes', 'rice bags', 'roof tiles', 'screws', 'shoes', 'spoon', 'spring rolls', 'stairs', 'stapler pins', 'straws', 'supermarket shelf', 'swans', 'tomatoes', 'watermelon', 'windows', 'zebras']
    tokenized_train_text_list=[]
    ...
    engine = inflect.engine()
    train_name_list=['alcohol bottles', 'alcohol bottles', 'alcohol bottles', 'alcohol bottles', 'alcohol bottles', 'bees', 'birthday candles', 'biscuits', 'boats', 'bottles', 'bowls', 'boxes', 'bread rolls', 'bricks', 'buffaloes', 'buns', 'calamari rings', 'candles', 'cans', 'caps', 'cars', 'cartridges', 'cassettes', 'cement bags', 'cereals', 'chewing gum pieces', 'chopstick', 'clams', 'coffee beans', 'coins', 'cotton balls', 'cows', 'cranes', 'crayons', 'croissants', 'crows', 'cupcake tray', 'cupcakes', 'cups', 'fishes', 'geese', 'gemstones', 'go game', 'goats', 'goldfish snack', 'ice cream', 'instant noodles', 'jade stones', 'jeans', 'kidney beans', 'kitchen towels', 'lighters', 'lipstick', 'm&m pieces', 'macarons', 'matches', 'meat skewers', 'mini blinds', 'mosaic tiles', 'naan bread', 'nails', 'nuts', 'onion rings', 'oranges', 'pearls', 'pencils', 'penguins', 'pens', 'people', 'peppers', 'pigeons', 'plates', 'polka dot tiles', 'potatoes', 'rice bags', 'roof tiles', 'screws', 'shoes', 'spoon', 'spring rolls', 'stairs', 'stapler pins', 'straws', 'supermarket shelf', 'swans', 'tomatoes', 'watermelon', 'windows', 'zebras']
    for train_name in train_name_list:
        if prompt == "plural":
            train_text = [template.format(engine.plural(train_name)) if engine.singular_noun(train_name) == False else template.format(train_name) for template in multi_plural_prompt_templates]
        elif prompt == "single":
            train_text = [template.format(train_name) if engine.singular_noun(train_name) == False else template.format(engine.plural(train_name)) for template in single_plural_prompt_templates]
        else:
            raise NotImplementedError

        tokenized_train_text = tokenize(train_text)
        tokenized_train_text_list.append(tokenized_train_text)

    return tokenized_train_text_list
