...
class SPTCLIPVisionTransformer(nn.Module):
    def __init__(self, input_resolution=384, patch_size=16, width=768, layers=12, heads=12, output_dim=512, drop_path_rate=0.1, out_indices=[5,6,7,8,11], pretrained=None, get_embeddings=True, 
        ...
        self.class_num=100  # Only 89 classes were used
    ...
    def _init_prompt(self, patch, num_tokens, prompt_dim, total_d_layer):
        ...
        if total_d_layer >= 0:
            self.prompt_embeddings = nn.Parameter(torch.zeros(self.class_num, 1, num_tokens, prompt_dim))
            # xavier_uniform initialization
            nn.init.uniform_(self.prompt_embeddings.data, -val, val)

            if total_d_layer > 0:  # noqa
                self.deep_prompt_embeddings = nn.Parameter(torch.zeros(self.class_num, total_d_layer, num_tokens, prompt_dim))
                # xavier_uniform initialization
            ...
            print("total_d_layer must bigger than 0")
            sys.exit(1)  
    ...
    def forward(self, x: torch.Tensor, _t: torch.Tensor,stage,class_ids,sim_list,class_index_list):
        x = self.conv1(x)
        B, C, H, W = x.shape
        ...
        if self.total_d_layer >=0:
            if stage=='train':
                ...
                x = torch.cat((
                    x[:, :1, :],
                        self.prompt_dropout(self.prompt_proj(vpts)) ,
                        x[:, 1:, :]
                    ), dim=1)
            else:
                visual_prompt_list=[]
                ...
                if stage=='train2':
                    ...
                    for jj in class_ids:
                        ...
                        # visual_prompt_list_class.append(self.returned_vpt[jj,0,:,:])
        ...
        features = []
        outs = []
        x, features = self.forward_deep_prompt(x, features, H, W, _t ,stage,class_ids,sim_list,class_index_list,outs_vpt_class,outs_vpt_weighted)
        ...
        return outs[0],outs_vpt_class,outs_vpt_weighted


    def forward_deep_prompt(self, embedding_output, features, H, W, _t,stage,class_ids,sim_list,class_index_list,outs_vpt_class,outs_vpt_weighted):
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                if stage =='train':
                    visual_prompt_list=[]
                    for j in class_ids:
                        visual_prompt_list.append(self.deep_prompt_embeddings[j,i-1,:,:])
                    vpts=torch.stack(visual_prompt_list,dim=0)

                    deep_prompt_emb = self.prompt_dropout(self.prompt_proj(vpts)).permute(1, 0, 2)
                    ...
                    visual_prompt_list=[]
                    for k in range(len(class_ids)):
                        visual_prompt_weighted = torch.einsum('i,ijk->jk', sim_list[k], self.deep_prompt_embeddings[class_index_list[k]][:,i-1,:,:]) 
                        visual_prompt_list.append(visual_prompt_weighted)
                    vpts=torch.stack(visual_prompt_list,dim=0)
                    if stage=='train2':
                        outs_vpt_weighted.append(vpts)
                    ...
                    if stage=='train2':
                        visual_prompt_list_class=[]
                        for jj in class_ids:
                            visual_prompt_list_class.append(self.deep_prompt_embeddings[jj,i-1,:,:])
                        visual_prompt_list_class=torch.stack(visual_prompt_list_class,dim=0) 
                        outs_vpt_class.append(visual_prompt_list_class)

                hidden_states = self.transformer.resblocks[i](hidden_states)
            
            elif i == self.deep_prompt_embeddings.shape[1]+1:
                hidden_states = torch.cat((
                    hidden_states[:1, :, :],
                    hidden_states[(1+self.num_tokens):, :, :]
                ), dim=0)
