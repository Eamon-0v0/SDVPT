...
class FSC147(Dataset):
    def __init__(self, 
        ...
        for image_name in data_split['train']:
            class_name=self.class_dict[image_name]
            if class_name not in self.train_name_list:
                self.train_name_list.append(class_name)
        self.train_name_list.sort()
        self.train_class_index_dic={}
        for i in range(len(self.train_name_list)):
            self.train_class_index_dic[self.train_name_list[i]]=i

        self.other_name_list=[]
        for class_name in self.all_classes:
            if class_name not in self.train_name_list:
                self.other_name_list.append(class_name)
        self.other_name_list.sort()
        self.other_class_index_dic={}
        for i in range(len(self.other_name_list)):
            self.other_class_index_dic[self.other_name_list[i]]=i
    ...
    def __getitem__(self, idx):
        ...
        if self.split=='train':
            class_id=self.train_class_index_dic[text]
        else:
            class_id=self.other_class_index_dic[text]
        ...
        if self.split == 'train' or (self.split == 'val' and self.resize_val):
            ...
            if self.use_additional_prompt:
                return sample['image'].float(), sample['gt_density'], sample['boxes'], sample['m_flag'], text, additional_prompt, class_id
            return sample['image'].float(), sample['gt_density'], sample['boxes'], sample['m_flag'], text, class_id
            ...
            return sample['image'].float(), sample['gt_map'], sample['boxes'], sample['pos'], text, class_id
