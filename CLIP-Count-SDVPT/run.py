...
def get_args_parser():
    ...
    parser.add_argument('--epochs', default=100, type=int,help = "CSPI epochs")
    parser.add_argument('--train2_epochs', default=200, type=int,help = "TGPR epochs")
    ...
    parser.add_argument('--data_path', default='./data/', type=str,
                        help='dataset path')
    parser.add_argument('--dataset_type', default="FSC", type = str, choices=["FSC","CARPK"])
...
class Model(LightningModule):
    def __init__(self, args, all_classes:List[str] = None):
        ...
        self.train_class_index_dic={'alcohol bottles': 0, 'baguette rolls': 1, 'balls': 2, 'bananas': 3, 'beads': 4, 'bees': 5, 'birthday candles': 6, 'biscuits': 7, 'boats': 8, 'bottles': 9, 'bowls': 10, 'boxes': 11, 'bread rolls': 12, 'bricks': 13, 'buffaloes': 14, 'buns': 15, 'calamari rings': 16, 'candles': 17, 'cans': 18, 'caps': 19, 'cars': 20, 'cartridges': 21, 'cassettes': 22, 'cement bags': 23, 'cereals': 24, 'chewing gum pieces': 25, 'chopstick': 26, 'clams': 27, 'coffee beans': 28, 'coins': 29, 'cotton balls': 30, 'cows': 31, 'cranes': 32, 'crayons': 33, 'croissants': 34, 'crows': 35, 'cupcake tray': 36, 'cupcakes': 37, 'cups': 38, 'fishes': 39, 'geese': 40, 'gemstones': 41, 'go game': 42, 'goats': 43, 'goldfish snack': 44, 'ice cream': 45, 'instant noodles': 46, 'jade stones': 47, 'jeans': 48, 'kidney beans': 49, 'kitchen towels': 50, 'lighters': 51, 'lipstick': 52, 'm&m pieces': 53, 'macarons': 54, 'matches': 55, 'meat skewers': 56, 'mini blinds': 57, 'mosaic tiles': 58, 'naan bread': 59, 'nails': 60, 'nuts': 61, 'onion rings': 62, 'oranges': 63, 'pearls': 64, 'pencils': 65, 'penguins': 66, 'pens': 67, 'people': 68, 'peppers': 69, 'pigeons': 70, 'plates': 71, 'polka dot tiles': 72, 'potatoes': 73, 'rice bags': 74, 'roof tiles': 75, 'screws': 76, 'shoes': 77, 'spoon': 78, 'spring rolls': 79, 'stairs': 80, 'stapler pins': 81, 'straws': 82, 'supermarket shelf': 83, 'swans': 84, 'tomatoes': 85, 'watermelon': 86, 'windows': 87, 'zebras': 88}
        self.train_name_list=['alcohol bottles', 'baguette rolls', 'balls', 'bananas', 'beads', 'bees', 'birthday candles', 'biscuits', 'boats', 'bottles', 'bowls', 'boxes', 'bread rolls', 'bricks', 'buffaloes', 'buns', 'calamari rings', 'candles', 'cans', 'caps', 'cars', 'cartridges', 'cassettes', 'cement bags', 'cereals', 'chewing gum pieces', 'chopstick', 'clams', 'coffee beans', 'coins', 'cotton balls', 'cows', 'cranes', 'crayons', 'croissants', 'crows', 'cupcake tray', 'cupcakes', 'cups', 'fishes', 'geese', 'gemstones', 'go game', 'goats', 'goldfish snack', 'ice cream', 'instant noodles', 'jade stones', 'jeans', 'kidney beans', 'kitchen towels', 'lighters', 'lipstick', 'm&m pieces', 'macarons', 'matches', 'meat skewers', 'mini blinds', 'mosaic tiles', 'naan bread', 'nails', 'nuts', 'onion rings', 'oranges', 'pearls', 'pencils', 'penguins', 'pens', 'people', 'peppers', 'pigeons', 'plates', 'polka dot tiles', 'potatoes', 'rice bags', 'roof tiles', 'screws', 'shoes', 'spoon', 'spring rolls', 'stairs', 'stapler pins', 'straws', 'supermarket shelf', 'swans', 'tomatoes', 'watermelon', 'windows', 'zebras']

    def training_step(self, batch, batch_idx):
        samples, gt_density, boxes, m_flag, prompt_gt, prompt_add, class_ids= batch
        class_ids=class_ids.to('cpu').numpy().tolist() 

        if self.current_epoch<=(self.args.contrast_pre_epoch+self.args.epochs):
            mode='train' #  'train'== CSPI
        else:
            mode='train2'#  'train2'== TGPR

        output, extra_out,outs_vpt_class,outs_vpt_weighted = self.model(samples, prompt_gt,mode,class_ids,self.train_name_list,self.train_class_index_dic, return_extra=True, coop_require_grad =  True)
        ...
        self.log('train_loss_mse', 1.0*loss)

        # Contrast pre-training as the the original paper
        if self.args.use_contrast and self.current_epoch <= self.args.contrast_pre_epoch:
            ...
            contrast_loss = self.contrastive_loss(patch_embedding, img_embedding, text_embedding, self.neg_prompt_embed,  gt_density.detach().clone(),prompt_gt)
            loss = args.w_contrast * contrast_loss
            self.log('train_loss_contrast', args.w_contrast * contrast_loss)
        
        # CSPI 
        elif self.current_epoch>self.args.contrast_pre_epoch and self.current_epoch<=(self.args.contrast_pre_epoch+self.args.epochs):
            loss=1.0*loss

        # TGPR 
        elif self.current_epoch>(self.args.contrast_pre_epoch+self.args.epochs):

            vpt_losses = [self.vpt_loss(outs_vpt_class[i], outs_vpt_weighted[i]) for i in range(len(outs_vpt_weighted))]
            vpt_losses = sum(vpt_losses) / len(vpt_losses)
            self.log('train_loss_vpt', 10.0*vpt_losses)
            loss=1.0*loss+10.0*vpt_losses
    ...
    def validation_step(self, batch, batch_idx):
        image, gt_density, boxes, m_flag, prompt,class_ids = batch
        
        class_ids=class_ids.to('cpu').numpy().tolist() 
        if not self.args.use_contrast:
            prompt = [f"a photo of {p}" for p in prompt]

        mode='val'   # As long as mode! = 'train' OR 'trian2', it is SDPE    
        ...
        class_ids=[ class_ids[0] for i in range(patches.shape[0])]

        output,outs_vpt_class,outs_vpt_weighted = self.model(patches, prompt,mode,class_ids,self.train_name_list,self.train_class_index_dic)
        ...
        return {"mae": batch_mae, "rmse": batch_rmse, "img": None, "pred": None, "gt": None, "heatmap_pred": None, "heatmap_gt": None, "prompt": prompt[0], "pred_cnts": pred_cnts, "gt_cnts": gt_cnts}
    ...
    def test_step(self, batch, batch_idx):
        if self.args.dataset_type=='FSC' or self.args.dataset_type == "COCO":
            ...
            image, gt_cnt = batch
            gt_cnt = gt_cnt.item()
            prompt = ["cars" for _ in range(image.shape[0])]
        ...
        batch_mae.append(cnt_err)
        batch_rmse.append(cnt_err ** 2)
        batch_nae.append(cnt_err/gt_cnt)
        batch_sre.append((cnt_err ** 2)/gt_cnt)
        pred_cnts.append(pred_cnt)
        gt_cnts.append(gt_cnt)
        ...
        if self.args.log_test_img:
            if cnt_err < 5:
                #log density
                log_dir = f"out/good_density/{args.exp_name}/"
                ...
                log_dir = f"out/good_pred/{args.exp_name}/"
                if not os.path.exists(log_dir):
                    os.makedirs(log_dir)
                #log overlay
                name = "good_{}_{:.2f}_gt_{:.2f}.jpg".format(prompt[0], pred_cnt, gt_cnt)
            ...
            if cnt_err > 100:
                ...
                log_dir = f"debug/bad_pred/{args.exp_name}/"
                if not os.path.exists(log_dir):
                    os.makedirs(log_dir)
        ...
        return {"mae": batch_mae, "rmse": batch_rmse,"nae": batch_nae, "sre": batch_sre, "img": img_log, "pred": pred_log_rgb, "gt": gt_log_rgb, "heatmap_pred": heatmap_pred, "heatmap_gt": heatmap_gt, "prompt": prompt[0], "pred_cnts": pred_cnts, "gt_cnts": gt_cnts}
    ...
    def test_epoch_end(self, outputs):
        ...
        test_nae = np.mean(all_nae)
        test_sre = np.sqrt(np.mean(all_sre))
        self.log('test_mae', test_mae)
        self.log('test_rmse', test_rmse)
        self.log('test_nae', test_nae)
        self.log('test_sre', test_sre)
    ...
    def configure_optimizers(self):
        ...
        schedular = torch.optim.lr_scheduler.StepLR(optimizer, step_size=50, gamma=0.8)
    ...
    def on_save_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
        ...
        if not self.args.unfreeze_vit :
            for k in list(checkpoint["state_dict"].keys()):
                if k.startswith("model.clip") or k.startswith("model.img_encoder.clip") or k.startswith("model.text_encoder.clip") or k.startswith("model.text_encoder_for_sim.clip")  or k.startswith("model.img_encoder.vit"):
...
if __name__ == '__main__':
    ...
    trainer = Trainer(
        ...
        precision=16, 
        max_epochs=args.epochs+args.contrast_pre_epoch+args.train2_epochs,
        logger=logger,
    ...
    if args.mode == "train":
        ...
        if args.dataset_type == "FSC" or args.dataset_type == "COCO": #CARPK  do not have val set
