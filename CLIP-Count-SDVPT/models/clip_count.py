...
class CLIPCount(nn.Module):
    def __init__(self, fim_depth:int=4, 
        ...
        self.text_encoder_for_sim=CLIPTextTransformer(self.clip,use_coop=False)
        # self.text_encoder_for_sim=CLIPTextTransformer(self.clip, use_coop=self.use_coop, n_ctx = self.coop_width)
        # --------------------------------------------------------------------------
        ...
        train_text_token = clip.tokenize([f"a photo of {p}" for p in train_name_list]).to('cuda')
        with torch.no_grad():
            self.train_text_embedding = self.text_encoder_for_sim(train_text_token).float()

    def forward_visual_encoder(self, x, text_embedding,mode,class_ids,sim_list,class_index_list):
        """
        ...
        # embed patches
        x = self.preprocess(x)
        _, cls_token, x,outs_vpt_class,outs_vpt_weighted = self.img_encoder(x, text_embedding,mode,class_ids,sim_list,class_index_list)
        return cls_token, x,outs_vpt_class,outs_vpt_weighted
    ...
    def forward(self, imgs, text,mode,class_ids,train_name_list,train_class_index_dic,return_extra:bool = False, coop_require_grad:bool = False):
        ...
        if mode=='train':
            ...
            text_token_for_sim = clip.tokenize([f"a photo of {p}" for p in text]).to(imgs.device)
            with torch.no_grad():
                text_embedding_sim = self.text_encoder_for_sim(text_token_for_sim).float()
            train_text_embedding=self.train_text_embedding

            text_embedding_expanded = text_embedding_sim  # [32, 1, 1, 512]
            train_text_embedding_expanded = train_text_embedding.squeeze().unsqueeze(0)  # [1, 89, 1, 512]
            sim = F.cosine_similarity(text_embedding_expanded, train_text_embedding_expanded, dim=-1)  # [32, 89]
            
            # apply self-mask
            new_sim = sim.detach().clone()
            if mode == 'train2': 
                class_ids_tensor = torch.tensor(class_ids, dtype=torch.long, device=sim.device)
                min_new_sim = new_sim.min(dim=1, keepdim=True).values  # [32, 1]
        ...
        cls_token, img_feat_patches,outs_vpt_class,outs_vpt_weighted = self.forward_visual_encoder(imgs, text_embedding,mode,class_ids,sim_list,class_index_list)
        pred_density, extra_out = self.forward_decoder(img_feat_patches, text_embedding, cls_token)  # [N, 384, 384]

        if return_extra:
            return pred_density, extra_out,outs_vpt_class,outs_vpt_weighted
        return pred_density,outs_vpt_class,outs_vpt_weighted
...
class CLIPViT(nn.Module):
    ...
    def __init__(self, 
        ...
        self.visual_prompt = None
        self.class_num=100 # Only 89 were used
        ...
        if use_vpt:
            ...
            vpt = torch.empty((self.class_num,vpt_depth, vpt_width, clip_embed_dim))
            nn.init.uniform_(vpt, -val, val)
            self.visual_prompt = nn.Parameter(vpt)


    def forward(self, image, text_embedding,mode,class_ids,sim_list,class_index_list):
        """
        ...
        if self.use_vpt:
            if mode=='train':
                visual_prompt_list=[]
                for j in class_ids:
                    visual_prompt_list.append(self.visual_prompt[j,0,:,:])
                ...
                for k in range(len(class_ids)):
                    visual_prompt_weighted = torch.einsum('i,ijk->jk', sim_list[k], self.visual_prompt[class_index_list[k]][:,0,:,:])  # [10, 512]
                    visual_prompt_list.append(visual_prompt_weighted)
                vpts=torch.stack(visual_prompt_list,dim=0) #[32,20,768]
                ...
                if mode=='train2':
                    ...
                    for jj in class_ids:
                        visual_prompt_list_class.append(self.visual_prompt[jj,0,:,:])
        ...
        if self.use_vpt and self.vpt_depth > 1:
            x = self.deep_vpt_forward(x,mode,class_ids,sim_list,class_index_list,outs_vpt_class,outs_vpt_weighted,text_embedding)
        ...
        return img_patches, x_cls, x ,outs_vpt_class,outs_vpt_weighted
    

    def deep_vpt_forward(self, embedding_output,mode,class_ids,sim_list,class_index_list,outs_vpt_class,outs_vpt_weighted, text_embdding = None, out_last = False):
        ...
        for i in range(transformer.layers):
            if i == 0:
                ...
                if mode=='train':
                    visual_prompt_list=[]
                    for j in class_ids:
                        visual_prompt_list.append(self.visual_prompt[j,i-1,:,:])
                    vpts=torch.stack(visual_prompt_list,dim=0)
                    deep_prompt_emb = self.vpt_dropout(self.vpt_proj(vpts)).permute(1, 0, 2)
                    # B, L, 768
                    ...
                    visual_prompt_list=[]
                    for k in range(len(class_ids)):
                        visual_prompt_weighted = torch.einsum('i,ijk->jk', sim_list[k], self.visual_prompt[class_index_list[k]][:,i-1,:,:])  # [10, 512]
                        visual_prompt_list.append(visual_prompt_weighted)
                    vpts=torch.stack(visual_prompt_list,dim=0)
                    ...
                    if mode=='train2':
                        visual_prompt_list_class=[]
                        for jj in class_ids:
                            visual_prompt_list_class.append(self.visual_prompt[jj,i-1,:,:])
                        visual_prompt_list_class=torch.stack(visual_prompt_list_class,dim=0) 
                        outs_vpt_class.append(visual_prompt_list_class)
